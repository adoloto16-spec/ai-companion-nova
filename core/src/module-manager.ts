...
export class ModuleManager{
  ...
  async disable(id:string){const entry=this.require(id);if(entry.state==="disabled")return;if(["running","degraded","ready"].includes(entry.state)){try{await entry.module.stop();}catch(error){entry.error=String(error);this.report("MODULE_STOP_FAILED",entry,error);}}this.transition(entry,"disabled");}
