...
export class ModuleManager{
  ...
  private async refreshHealth(entry:Entry):Promise<HealthStatus>{
    ...
    catch(error){entry.error=String(error);if(entry.state==="running"||entry.state==="degraded")this.transition(entry,"degraded");else if(entry.state!=="disabled")this.transition(entry,"error");this.report("MODULE_HEALTH_FAILED",entry,error);return{status:"degraded",message:String(error)};}
  }
