import type {
  ActionInvocation,ActionRequest,ActionTarget,ActionTargetResolver,ActorCredential,ActorIdentity,ActorIdentityResolver,
  CapabilityContext,ConfirmationService,Permission,PermissionDecision,RiskPolicy,ToolDefinition
...
export function normalizeFilesystemPath(value:string):string{
  ...
  const prefix=drive?.[1]?drive[1].toUpperCase()+"/":replaced.startsWith("/")?"/":"";
...
export function filesystemPathAllowed(actual:string,roots:readonly string[],canonicalized:boolean):boolean{
  ...
  return roots.some(root=>{
    const normalized=normalizeFilesystemPath(root).replace(/\/$/,"").toLowerCase();
    return candidate===normalized||candidate.startsWith(normalized+"/");
  });
}

export function applicationAllowed(
  applicationId:string,
  windowId:string|undefined,
  allowed:readonly string[],
  windows:readonly string[]|undefined
):boolean{
...
export class FilesystemTargetResolver implements ActionTargetResolver{
  readonly id:string;
  constructor(id="filesystem.path",private readonly canonicalize?:(path:string)=>Promise<string|undefined>){
    this.id=id;
  }
  async resolve(request:ActionRequest):Promise<ActionTarget>{
...
export class InMemoryActorIdentityResolver implements ActorIdentityResolver{
  private readonly identities=new Map<string,ActorIdentity>();

  register(credential:ActorCredential,identity:ActorIdentity):void{
    this.identities.set(credential.token,identity);
  }

  async resolve(credential:ActorCredential):Promise<ActorIdentity|undefined>{
    return this.identities.get(credential.token);
  }
}

export class DefaultRiskPolicy implements RiskPolicy{
  canonicalRisk(tool:ToolDefinition){return tool.risk;}
  requiresConfirmation(tool:ToolDefinition){
    return tool.confirmation==="policy"&&(tool.risk==="high"||tool.risk==="critical");
  }
}

export class DefaultConfirmationService implements ConfirmationService{
  constructor(private readonly approve:(invocation:ActionInvocation,actor:ActorIdentity,tool:ToolDefinition,target:ActionTarget)=>Promise<boolean>){}
  confirm(invocation:ActionInvocation,actor:ActorIdentity,tool:ToolDefinition,target:ActionTarget){
    return this.approve(invocation,actor,tool,target);
  }
}
...
export class InMemoryPermissionService{
  ...
  add(rule:Permission):void{this.rules.push({...rule});}
  ...
  async check(actor:ActorIdentity,tool:ToolDefinition,target:ActionTarget):Promise<PermissionDecision>{
    const matches=this.rules.filter(rule=>
      rule.subject===actor.actorId&&
      rule.resourceType===tool.resourceType&&
      rule.action===tool.action
    );
    for(const rule of [...matches].reverse()){
      if(!this.matchesScope(rule,target))continue;
      return rule.effect==="allow"
        ? {allowed:true,reason:"Permission allowlisted.",scopeMatched:true}
        : {allowed:false,reason:"Permission explicitly denied.",scopeMatched:true};
    }
  ...
  private matchesScope(rule:Permission,target:ActionTarget):boolean{
    ...
    if(target.kind==="application"){
      return !!rule.scope.applications&&
        applicationAllowed(target.applicationId,target.windowId,rule.scope.applications,rule.scope.windows);
    }
    return false;
  }
}
