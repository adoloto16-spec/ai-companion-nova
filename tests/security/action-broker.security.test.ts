import {
  DefaultActionBroker,DefaultConfirmationService,DefaultRiskPolicy,InMemoryAuditService,InMemoryActorIdentityResolver,
  InMemoryPermissionService,InMemoryToolRegistry,BrowserTargetResolver,FilesystemTargetResolver,ApplicationTargetResolver
} from "../../core/src";
...
function makeBroker(
  ...
  resolver:{id:string;resolve(request:ActionRequest,tool:ToolDefinition):Promise<ActionTarget>},
  driver:{id:string;execute(request:ActionRequest,target:ActionTarget):Promise<unknown>},
  scope:Parameters<InMemoryPermissionService["add"]>[0]["scope"],
  ...
  actorResolver.register(credential,{
    actorId:"character",
    actorType:"module",
    moduleId:"character.fake",
    trusted:true,
    capabilities:["browser.navigate","filesystem.write","computer.control"]
  });
  tools.register(definition,driver);
  permissions.add({
    id:"allow",
  ...
  const broker=new DefaultActionBroker({
    ...
    confirmation:new DefaultConfirmationService(async()=>confirmationResult),
    audit,
    schemaValidator:new MinimalJsonSchemaValidator(),
    targetResolvers:new Map([[resolver.id,resolver]]),
    actorResolver
  });
  return {broker,audit,actorResolver};
}

const request=(tool:string,argumentsValue:Record<string,unknown>):ActionInvocation=>({
  request:{id:"a1",schemaVersion:FOUNDATION_SCHEMA_VERSION,tool,arguments:argumentsValue},
  credential
});
...
async function normalDomainScenario(){
  const {broker}=makeBroker(browserDefinition,new BrowserTargetResolver("browser.url"),{id:"driver",async execute(_r,target){return target}},{domains:["youtube.com"]});
...
async function unknownCredentialScenario(){
  const {broker}=makeBroker(browserDefinition,new BrowserTargetResolver("browser.url"),{id:"driver",async execute(){return {ok:true}}},{domains:["youtube.com"]});
...
async function riskDowngradeScenario(){
  const definition:ToolDefinition={
    ...browserDefinition,
    id:"filesystem.delete",name:"filesystem.delete",risk:"critical",
    requiredCapabilities:["filesystem.write"],resourceType:"filesystem",action:"filesystem.delete",
    targetResolverId:"filesystem.path",confirmation:"policy",
    parameters:{type:"object",properties:{path:{type:"string"}},required:["path"],additionalProperties:false}
  };
  const resolver=new FilesystemTargetResolver("filesystem.path",async path=>path);
  const {broker,audit}=makeBroker(definition,resolver,{id:"driver",async execute(){return {deleted:true}}},{roots:["D:/AI_Girl/"]},false);
...
async function capabilityScenario(){
  const definition:ToolDefinition={
    ...browserDefinition,id:"filesystem.write",name:"filesystem.write",
    requiredCapabilities:["filesystem.write"],resourceType:"filesystem",action:"filesystem.write",
    targetResolverId:"filesystem.path",
    parameters:{type:"object",properties:{path:{type:"string"}},required:["path"],additionalProperties:false}
  };
  const resolver=new FilesystemTargetResolver("filesystem.path",async path=>path);
  const harness=makeBroker(definition,resolver,{id:"driver",async execute(){return {ok:true}}},{roots:["D:/AI_Girl/"]});
...
async function filesystemScenario(){
  ...
  const resolver=new FilesystemTargetResolver("filesystem.path",async path=>path);
  const {broker}=makeBroker(definition,resolver,{id:"driver",async execute(){return {ok:true}}},{roots:["D:/AI_Girl/"]});
...
async function applicationScenario(){
  const definition:ToolDefinition={
    ...browserDefinition,id:"application.control",name:"application.control",risk:"high",
    requiredCapabilities:["computer.control"],resourceType:"application",action:"application.control",
    targetResolverId:"application.window",
    parameters:{type:"object",properties:{applicationId:{type:"string"},windowId:{type:"string"}},required:["applicationId"],additionalProperties:false}
  };
  const {broker}=makeBroker(definition,new ApplicationTargetResolver("application.window"),{id:"driver",async execute(){return {ok:true}}},{applications:["chrome.exe"]},true);
...
async function parameterAndDriverScenario(){
  const resolver=new BrowserTargetResolver("browser.url");
  const tools=new InMemoryToolRegistry();
  const permissions=new InMemoryPermissionService();
  const audit=new InMemoryAuditService();
  const actorResolver=new InMemoryActorIdentityResolver();
  actorResolver.register(credential,{actorId:"character",actorType:"module",moduleId:"character.fake",trusted:true,capabilities:["browser.navigate"]});
  ...
  const broker=new DefaultActionBroker({
    toolRegistry:tools,permissions,foreground,riskPolicy:new DefaultRiskPolicy(),
    confirmation:new DefaultConfirmationService(async()=>true),audit,
    schemaValidator:new MinimalJsonSchemaValidator(),targetResolvers:new Map([[resolver.id,resolver]]),actorResolver
  });
...
void (async()=>{
  for(const [name,test] of [
    ["normal domain",normalDomainScenario],["forged scope",forgedScopeScenario],["unknown credential",unknownCredentialScenario],
    ["risk downgrade",riskDowngradeScenario],["capability enforcement",capabilityScenario],["filesystem scope",filesystemScenario],
    ["application scope",applicationScenario],["parameter/driver security",parameterAndDriverScenario],["postcondition",postconditionScenario]
